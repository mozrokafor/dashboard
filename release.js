...
async function generateWorkflowStats() {
  ...
  const awfsPath = "./data/release/allworkflowstats.json";
...
async function getAllWorkflows() {
  const awfPath = "./data/release/allworkflows.json";
  await checkPath("./data/release");
  await checkNestedPath("./data/release/checkruns");
  await checkNestedPath("./data/release/workflowruns");
...
async function updateAllWorkflowRuns() {
  ...
  for (let workflow of workflowsArray) {
    ...
    const existingWorkflowRunsArray = JSON.parse(
      fs.readFileSync(
        `./data/release/workflowruns/workflowruns-${workflow.workflow_id}.json`,
        "utf8"
      )
    ).workflow_runs;
...
async function generateTestHistoryInfo() {
  let return_data = [];
  const frPath = "./data/release/workflowruns/functional-workflow-runs.json";
  const thPath = "./data/release/checkruns/test-history.json";
